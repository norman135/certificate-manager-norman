...
const SideNavItem: React.FC <SideNavItemProps> = ({svgIconPath, svgVB, itemName, subMenu, selected, index, onclick, onsubclick}): JSX.Element => {
    return (
        <div className="side-nav-item" onClick={() => onclick(index)}>
            <div className={selected ? "side-nav-item-desc sni-selected" : "side-nav-item-desc"}>
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox={svgVB}>
                    <path d={svgIconPath}/>
                </svg>
...
const SideNav: React.FC <SideNavProps> = ({setVisibleContent}): JSX.Element => {
...
const SideBar: React.FC <SideNavProps> = ({setVisibleContent}): JSX.Element => {
    return (
